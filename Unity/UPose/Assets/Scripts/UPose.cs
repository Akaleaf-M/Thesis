...
{
    [Header("CSV Rotation Playback")]
    public bool useCSV = false;          // 打开后使用 CSV 代替 UDP
    public string csvFilePath;           // 旋转 CSV 的路径
    public float csvFPS = 30f;           // 播放帧率

    private List<Quaternion[]> csvRotFrames;
    private int csvCurrentFrame = 0;
    private float csvTimeAccumulator = 0f;
    private int lastCsvFrameIndex = -1;

    public string host = "127.0.0.1"; // This machines host.
    public int port = 53000; // Must match the Python side.
    ...
    // CSV 格式：
    // frame, pelvis_x,pelvis_y,pelvis_z,pelvis_w,
    ...
    //        l_knee_x,...,
    //        r_knee_x,...
    private void LoadCSVRotations(string path)
    {
        csvRotFrames = new List<Quaternion[]>();

        if (string.IsNullOrEmpty(path))
        {
            Debug.LogError("CSV path is empty.");
            return;
        }
        ...
        {
            Debug.LogError("Rotation CSV file not found at: " + fullPath);
            return;
        }

        Debug.Log("Loading rotation CSV from: " + fullPath);

        var lines = File.ReadAllLines(fullPath);
        if (lines.Length <= 1)
        {
            Debug.LogWarning("Rotation CSV has no data lines.");
            return;
        }
        ...
        for (int li = 1; li < lines.Length; li++)
        {
            ...
            // 1(frame) + 10 joints * 4 = 41
            if (tokens.Length < 1 + 10 * 4) continue;

            int idx = 1; // 跳过 frame

            Quaternion[] frameRots = new Quaternion[LANDMARK_COUNT];
            for (int i = 0; i < LANDMARK_COUNT; i++)
            {
                frameRots[i] = Quaternion.identity;
            }

            // 对齐 Python 端导出顺序（和 mprot mr[] 一致）
            frameRots[(int)Landmark.PELVIS] = new Quaternion(
                float.Parse(tokens[idx + 0]),
                float.Parse(tokens[idx + 1]),
                float.Parse(tokens[idx + 2]),
                float.Parse(tokens[idx + 3])
            );
            idx += 4;

            frameRots[(int)Landmark.SHOULDER_CENTER] = new Quaternion(
                float.Parse(tokens[idx + 0]),
                float.Parse(tokens[idx + 1]),
                float.Parse(tokens[idx + 2]),
                float.Parse(tokens[idx + 3])
            ...
            frameRots[(int)Landmark.LEFT_SHOULDER] = new Quaternion(
                float.Parse(tokens[idx + 0]),
                float.Parse(tokens[idx + 1]),
                float.Parse(tokens[idx + 2]),
                float.Parse(tokens[idx + 3])
            ...
            frameRots[(int)Landmark.RIGHT_SHOULDER] = new Quaternion(
                float.Parse(tokens[idx + 0]),
                float.Parse(tokens[idx + 1]),
                float.Parse(tokens[idx + 2]),
                float.Parse(tokens[idx + 3])
            ...
            frameRots[(int)Landmark.LEFT_ELBOW] = new Quaternion(
                float.Parse(tokens[idx + 0]),
                float.Parse(tokens[idx + 1]),
                float.Parse(tokens[idx + 2]),
                float.Parse(tokens[idx + 3])
            ...
            frameRots[(int)Landmark.RIGHT_ELBOW] = new Quaternion(
                float.Parse(tokens[idx + 0]),
                float.Parse(tokens[idx + 1]),
                float.Parse(tokens[idx + 2]),
                float.Parse(tokens[idx + 3])
            ...
            frameRots[(int)Landmark.LEFT_HIP] = new Quaternion(
                float.Parse(tokens[idx + 0]),
                float.Parse(tokens[idx + 1]),
                float.Parse(tokens[idx + 2]),
                float.Parse(tokens[idx + 3])
            ...
            frameRots[(int)Landmark.RIGHT_HIP] = new Quaternion(
                float.Parse(tokens[idx + 0]),
                float.Parse(tokens[idx + 1]),
                float.Parse(tokens[idx + 2]),
                float.Parse(tokens[idx + 3])
            ...
            frameRots[(int)Landmark.LEFT_KNEE] = new Quaternion(
                float.Parse(tokens[idx + 0]),
                float.Parse(tokens[idx + 1]),
                float.Parse(tokens[idx + 2]),
                float.Parse(tokens[idx + 3])
            ...
            frameRots[(int)Landmark.RIGHT_KNEE] = new Quaternion(
                float.Parse(tokens[idx + 0]),
                float.Parse(tokens[idx + 1]),
                float.Parse(tokens[idx + 2]),
                float.Parse(tokens[idx + 3])
            ...
            csvRotFrames.Add(frameRots);
        }

        Debug.Log("Loaded " + csvRotFrames.Count + " rotation frames from CSV.");
        csvCurrentFrame = 0;
        csvTimeAccumulator = 0f;
        body.format = 0; //  mprot 模式
    }

    private void UpdateBodyFromCSVRotations()
    {
        if (csvRotFrames == null || csvRotFrames.Count == 0) return;

        csvTimeAccumulator += Time.deltaTime;
        float frameDuration = 1f / Mathf.Max(1f, csvFPS);

        while (csvTimeAccumulator >= frameDuration)
        {
            csvTimeAccumulator -= frameDuration;
            csvCurrentFrame++;
            if (csvCurrentFrame >= csvRotFrames.Count)
            {
                csvCurrentFrame = 0; // 循环播放
            }
        }

        if (csvCurrentFrame != lastCsvFrameIndex)
        {
            lastCsvFrameIndex = csvCurrentFrame;
            frame_counter++;        //  让 PoseMemory 知道“新帧来了”
        }

        var frameRots = csvRotFrames[csvCurrentFrame];

        for (int i = 0; i < LANDMARK_COUNT; i++)
        {
            body.rotations[i] = frameRots[i];
        }
