...
unity_port = int(sys.argv[2]) if len(sys.argv) > 2 else 52733
agg_port = int(sys.argv[3]) if len(sys.argv) > 3 else 52833
...
unity_client = ClientUDP('127.0.0.1', unity_port)
unity_client.start()

agg_client = ClientUDP('127.0.0.1', agg_port)
agg_client.start()

print(f"[run_mediapipe] cam={camera_id}, unity_port={unity_port}, agg_port={agg_port}")
...
if not cap.isOpened():
    raise RuntimeError(f"Failed to open camera index {camera_id}")
...
with mp_pose.Pose(
    ...
    while cap.isOpened():
        ...
        cv2.imshow(f'MediaPipe Pose cam{camera_id}', image)
        ...
        if results.pose_world_landmarks:
            ...
            unity_client.sendMessage(data)
            agg_client.sendMessage(data)
