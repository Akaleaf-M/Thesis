...
| TODO-020 | `docs/ARCHITECTURE.md`, `docs/RUNBOOK.md` | `WaterfallA` still needs direct VCV / rhythm control design and implementation. `WaterfallB` should remain audio-reactive only and should not receive VCV control messages. | important | artist decision / code | Define a minimal VCV -> Unity control protocol for `WaterfallA`, using a port range separate from MediaPipe / UPose. Candidate controls: `SetIntensity`, `SetSpeedMultiplier`, `SetDensityMultiplier`, `TriggerPulse`, `SetGlitchAmount`, and `TriggerAccent`. | no |
