...
| TODO-014 | `docs/RUNBOOK.md` | 确认 current machine / Mac Studio 的 actual camera index mapping。 | critical | hardware test | 在 Windows 上运行 `python list_cameras_windows.py`，在 Mac Studio 上运行 `python list_cameras_mac.py`，确认 P1-P4 对应 camera index；如与默认 `0,1,2,3` 不同，更新 `start_pose_system_windows.bat` 或 `start_pose_system_mac.sh` 中的 `CAM_P1` 到 `CAM_P4`。 | yes |
